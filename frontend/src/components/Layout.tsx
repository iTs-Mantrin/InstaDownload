...
const NAV_ITEMS: NavItem[] = [
  ...
  { icon: <FacebookIcon />, labelKey: 'nav.facebook', path: '/' },
  { icon: <TikTokIcon />, labelKey: 'nav.tiktok', path: '/' },
  { icon: <PinterestIcon />, labelKey: 'nav.pinterest', path: '/' },
]
...
function DesktopNavItem({ item, currentPath }: { item: NavItem; currentPath: string }) {
  ...
  const active = isActive(item.path, currentPath) || childActive(item.children, currentPath)

  // Group children for visual sections
  ...
  return (
    <div className="relative group">
      ...
      >
        ...
        {item.children && (
          <svg className="w-3 h-3 mt-0.5 opacity-60 group-hover:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {item.children && groups && (
        <div className="absolute left-0 mt-1 w-64 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 translate-y-1 group-hover:translate-y-0 z-50">
              ...
              <div key={groupKey}>
                ...
                {children.map((child) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`w-1.5 h-1.5 rounded-full shrink-0 transition-colors ${childActiveFlag ? 'bg-blue-500' : 'bg-slate-300 dark:bg-slate-600'}`} />
...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white text-slate-900 dark:bg-slate-900 dark:text-slate-100 transition-colors duration-300">
      {/* Navbar */}
      <header className="border-b border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md sticky top-0 z-50 transition-colors duration-300">
        <nav className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-0.5">
            {NAV_ITEMS.map((item) => (
              <DesktopNavItem key={item.labelKey} item={item} currentPath={location.pathname} />
            ))}
          </div>
        ...
        {menuOpen && (
          <div
            ref={mobileRef}
            className="md:hidden border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 transition-colors duration-300 animate-fade-in"
          >
            <div className="px-3 py-3 space-y-1 max-h-[70vh] overflow-y-auto">
              {NAV_ITEMS.map((item) => {
                const active = isActive(item.path, location.pathname) || childActive(item.children, location.pathname)
                ...
                return (
                  <div key={item.labelKey}>
                    ...
                    {hasChildren && expanded && (
                      <div className="ml-6 mt-1 mb-1 space-y-0.5 border-l-2 border-slate-200 dark:border-slate-700 pl-3 overflow-hidden transition-all duration-200">
